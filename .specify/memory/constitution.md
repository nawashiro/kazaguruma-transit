...
- ルビの補助テキストを除き、ユーザー向けテキストの算出フォントサイズは 16px 未満にしてはならない。
- UI のアイコンは Lucide（`lucide-react`）のアイコンコンポーネントだけを使用しなければならない。手書きのインライン SVG、Heroicons、react-icons などは使用してはならない。
...
- 制約のある手段で無理のない移動計画を検索できること。
- 移動計画に含まれる障害を事前に情報提供できること。

### 範囲外

- 後方互換性を目的とした実装を追加してはならない。KISS 原則を優先し、旧実装を温存するフォールバックを新たに作らない。
- 本プロジェクトは個人の私財で保守する小規模な非公式アプリケーションであり、権威ある行政システムとしての完全な互換性保証を範囲に含めない。
- 既存設計を置き換えるときは、仕様と現行実装を確認したうえで、不要な旧経路を破壊的に削除する。複雑性を増やす保険的な二重実装を残してはならない。
...
Nostr readは`specs/022-nostr-discussion-read-coordinator`を正本とする。詳細は同仕様のデータモデルと契約を参照する。
...
- リリースブランチ: `master`
- 機能開発は `dev` から始める。
- 開発は TDD で行う。テストを先に作成し、RED を確認してから本番実装へ進み、GREEN とリファクタリングを確認する。
- 実装タスクはサブエージェントへ委任する。親エージェントは、委任前に受入条件と書込境界を定義し、委任後に実際の変更と検証結果を確認する。
- テスト実装タスクの直後に、そのテストを対象とするレビュータスクを置く。レビュータスクもサブエージェントへ委任し、テストが仕様を過不足なく表現していることを本番実装前に確認する。
- 親エージェントは、受入条件、各サブエージェントの書込境界、RED/GREEN の実行結果、最終検証結果を一元管理する。サブエージェントの自己申告だけを完了根拠にしてはならない。
...
- この憲章は Spec Kit のガバナンス文書であり、変更時は本ファイル先頭の Sync Impact Report に変更理由、版変更、追加・変更・削除した項目、未完了のフォローアップを記録する。
- 憲章を変更するときは、既存の原則とリポジトリの実装・文書を確認し、意味のある原則の追加・拡張は MINOR、後方互換でない原則の変更・削除は MAJOR、文言の明確化は PATCH としてバージョンを更新する。
- 変更後は、未置換のテンプレートプレースホルダー、日付形式、版番号、該当する検証コマンドを確認する。feature の計画・実装・レビューでは、該当する原則への適合結果を記録する。
