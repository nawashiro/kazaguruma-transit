...
- Reason: 通常のユーザー向けテキストに `text-base` を原則とする規範を追加。
- Modified Principles:
  * Accessibility & UX: 通常テキストの既定サイズを `text-base` として明文化。
- Added Sections: なし
- Removed Sections: なし
- Templates Status:
  ✅ .specify/templates/plan-template.md: Constitution Check は本文の WCAG gate と整合
  ✅ .specify/templates/spec-template.md: 新たな必須セクションは不要
  ✅ .specify/templates/tasks-template.md: 既存のUI検証タスク構成で対応可能
  ✅ .specify/templates/commands/: ディレクトリが存在せず、更新対象なし
  ✅ AGENTS.md: 実務上の正本へ同じ規則を同期
...
- 本文、説明文、ラベルなどの通常のユーザー向けテキストは、原則として Tailwind CSS の
  `text-base`（`1rem`、通常 16px）を使用する。情報の階層やコンポーネントの役割上、
  より小さい文字が必要な場合に限り、14px 未満禁止と WCAG 2.2 AA の要件を維持した上で
  `text-sm` などを使用できる。
...
**Version**: 1.9.0 | **Ratified**: 2026-01-13 | **Last Amended**: 2026-07-18
