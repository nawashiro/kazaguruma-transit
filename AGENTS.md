...
- Operators must provide `app-config.json` for deployment-specific public application settings before running lifecycle or build commands. Lifecycle and Docker checks only verify that it exists and never generate it from the tracked example. The Quality Gate CI may create an ephemeral copy explicitly for its checkout; do not store or expose real deployment-specific values in the repository. Keep `transit-config.json` as the server-side GTFS secret, and add `.env.local` with at least `GOOGLE_MAPS_API_KEY` plus any other server/deployment-only settings from the README.
