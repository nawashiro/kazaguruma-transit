...
- Create or edit `app-config.json` for public application settings, keep `transit-config.json` as the server-side GTFS secret, and add `.env.local` with at least `GOOGLE_MAPS_API_KEY` plus any other server/deployment-only settings from the README.
