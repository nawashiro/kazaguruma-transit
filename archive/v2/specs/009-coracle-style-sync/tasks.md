...
- [X] T065 [P] 共通moderation snapshot fixtureで、一覧・管理・BusStop系が承認遅延・別relay・空streamで`unknown`を維持し、承認到着後に`approved`へ更新することを検証する
- [X] T066 [P] `src/components/discussion/__tests__/EvaluationComponent.test.tsx` に、`unknown`投稿を確定的に未承認除外せずsnapshot更新後に再評価できるテストを追加する
...
- [X] T070 `src/components/discussion/EvaluationComponent.tsx` と統計入力を、`approved`確定投稿と`unknown`保留投稿に分離し、承認snapshot更新後に再計算する
...
- [X] T074 全Discussion画面の同一fixture横断テスト、権限回帰、撤回、重複排除、フォーム/フォーカス維持を実行する
- [X] T075 `npx tsc --noEmit`、`npm run lint`、`npm test`、`npm run build`を実行し、結果を`quickstart.md`へ追記する
