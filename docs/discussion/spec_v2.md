...
## UI 実装ポリシー（DaisyUI 第一選択）

- 同等責務のUIは DaisyUI コンポーネントを第一選択とする（`btn` `input` `textarea` `badge` `alert` `loading` `tabs` `modal` `timeline`）。
- 独自実装は DaisyUI で代替不可能な責務に限定する。
- disabled操作には理由文を必ず併記し、アクセシビリティ属性を維持する。
...
2. 会話画面 `discussions/[naddr]`: naddr を受け取り、該当 kind:34550 を表示する。
   ...
   2. 監査ログも修正する必要がある。プロファイルは取得せず、「作成者」「モデレーター」の badge（DaisyUI クラス）を表示する。これは、管理者とスタッフ以外のユーザー名を明かさない方針による。
...
1. ユーザーは会話一覧から会話作成画面へアクセスし、会話を作成・会話一覧への追加をリクエストする。
   ...
   2. 入力を行う。項目に簡潔な解説があるといい。
   3. 作成ボタンを押すと、会話一覧への掲載リクエストと作成が同時に行われる。
   4. 完了メッセージが表示される。
      1. 会話が作成されました。すぐに開始できます。URL を共有すれば、仲間を呼び込めます。
      2. まずは話題の呼び水として、10 個程度の書き込みをしてみてください。なるべく簡単なものを書き込むと、他の人が参加しやすくなります。
...
- [NIP-18](NIP-18.md)に定義されている q タグを使用する。内容は[NIP-01](NIP-01.md)に定義された置換可能イベントの`34550:f723...:abcd`にする。
...
以下の環境変数が必要だと思われる。不要な環境変数は無くすべき。

- 会話一覧画面の kind:34550（naddr 形式）
- 管理者のユーザー ID（npub）
