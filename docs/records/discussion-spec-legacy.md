> **Status:** Historical — Legacy specification（旧仕様・履歴）
> **記録時点:** 原文に明記なし
> **Scope:** 現行仕様・手順ではありません。`nostr-tools`、旧環境変数、旧`[id]` route、本文の相対リンクは旧仕様の記録です。
...
Nostr プロトコルを使用する。投稿はリレーサーバーに分散して保存され、削除に耐性を持つ。電子署名によって改ざんに耐性を持つ。基本プロトコルフローは [NIP-01](../discussion/NIP-01.md) を参照のこと。[nostr-tools](../discussion/nostr-tools.md) を SDK として使う。[NIP-72](../discussion/NIP-72.md) は Reddit 形式のディスカッションに関する仕様だ。[nosskey-sdk](../discussion/nosskey.md) を使用して、アカウント作成・鍵管理を Passkey で行う。
