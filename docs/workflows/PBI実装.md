...
<workflow>
1. ./docs/knowledge/JIRA.mdを読んで現在のスプリントのTODOステータスのPBIを取得する。必要に応じてMCPを実行する。
2. 優先順位の高いPBIを1つ選択する。
3. 選択したPBIのステータスを「進行中」に更新する。
4. 選択したPBIについて以下の情報をまとめる（コメントも併せて確認してください）：
   1. PBIタイトル
   2. PBI URL
   3. 実装内容の詳細
   4. 受け入れ基準
...
5. 実装計画を立てる：
   ...
   2. 実装手順
   3. テスト計画
6. コードベースを調査し、実装に必要な変更箇所を特定する。
7. テスト駆動開発の手法に従い、まずテストコードを作成する。
8. テストが失敗することを確認後、実装コードを作成する。
9.  テストが成功することを確認する。
10. 必要に応じてリファクタリングを行う。
11. 実装が完了したらレビューのためにユーザーに確認を依頼する。
12. レビューで指摘された修正点があれば対応する。
13. 最終的な実装を「チケットID-コミットメッセージ」でコミットし、JIRAのPBIステータスを「レビュー待ち」に更新する。
</workflow>
