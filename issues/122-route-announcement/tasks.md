...
```text
T001 → T002 → T003 → T004 → T005 → T006 → T007 → T008 → T009 → T008R → T009R → T010 → T010C → T010CR → T011 → T012 → T013 → T014 → T015 → T016 → T017 → T018 → T019 → T020 → T021
```
...
- T011〜T014、T019〜T021は親が担当し、サブエージェントの自己申告だけで完了扱いにしない。
...
## Phase 6: 初回スタイル追補（ユーザー指摘により不採用）

- [x] **T015 [SUPERSEDED]** 既存Infoテストへ`inline-block` class assertionを一時追加し、1 suite / 12 passed・1 failedのREDを確認した。
- [x] **T016 [SUPERSEDED]** 上記assertionのfresh reviewはPASSだったが、ユーザーの方針によりstyleをテストで固定しない構成へ変更するため、最終契約としては採用しない。
- [x] **T017 [SUPERSEDED]** Info iconへ`inline-block`を追加した一時実装は、ユーザーの指定する`card-title flex`構造へ置き換えた。
- [x] **T018 [SUPERSEDED]** 一時実装のPuppeteer確認は、最終実装の根因調査資料としてのみ保持する。最終styleの合否をcomputed styleテストで固定しない。

## Phase 7: ユーザー指摘後の最終修正

- [x] **T019 [USER-CORRECTION-VERIFIED]** ユーザー指摘に従い、`src/app/__tests__/page.test.tsx`から`inline-block` assertionとカード装飾／位置のstyle専用テストを削除した。既存のh2、ARIA、Infoの装飾扱い、設定link、旧表示撤去、Home操作の意味論・ロジックテストは維持した。
- [x] **T020 [MINIMAL-STRUCTURE-VERIFIED]** `src/components/features/Announcement.tsx`を`h2.card-title.flex.gap-0`、Infoアイコン、`span.ruby-text.gap-0`の構造へ変更した。既存の`card-title`契約は`inline`または明示的な`flex`を許容するよう一般化し、新しいstyle専用suiteは追加していない。
- [x] **T021 [VERIFY-FINAL-VERIFIED]** 関連focused Jestは3 suites / 17 tests passed、全Jestは2 skipped / 145 passed suites・13 skipped / 914 passed tests、strict TypeScript・lint・buildはexit 0だった。Puppeteerの候補測定では`h2.card-title.flex`が`display:flex`となることを確認した。既存warningとGTFS設定不足表示は差分由来ではない。
