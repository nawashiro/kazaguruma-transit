...
- [x] 手書きインライン SVG を共通UIと対象画面から除去し、装飾・意味伝達の扱いを統一する。
- [x] ログインモーダルの `area-selected` 誤記を修正し、タブ状態をプログラムから判定可能にする。
- [x] 出発地選択の fieldset と凡例の関連付けを適正化する。
- [x] 場所検索の住所入力に可視ラベルを関連付ける。
- [x] サイドバーのメニュー開閉をキーボードで確実に操作できるようにする。
- [x] タブ・tabpanel の役割、選択状態、関連付け、フォーカス管理を全箇所で統一する。
- [x] 44px 未満のボタン・操作領域を修正し、対象ユーザーを識別できる名前を付ける。
- [x] 通常テキスト、アイコン、境界線、フォーカス表示のコントラストをテーマ別に測定し、基準未達を修正する。
- [x] 200% 拡大と狭い画面幅でのリフローを確認する。
...
- 実装後確認: 手書きSVG・`area-selected` の静的契約テスト、全テスト（83成功、430テスト成功、17スキップ）、型チェック、lint、production build が成功した。lint は既存の警告のみ残る。
