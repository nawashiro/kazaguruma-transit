...
- **Button contract:** inspect ordinary DaisyUI buttons and links used as controls to ensure their rendered/public classes explicitly enforce canonical named `text-base` or an equivalent 16px size. Non-canonical arbitrary font-size notation such as `text-[...]`, `text-(...)`, or `text-[theme(...)]` is outside this contract, as the analogous arbitrary color notation is outside the color contract. The current production source inventory has no composition-helper callsites, so loop-binding shadowing of helper names is intentionally out of scope for this slice. Do not classify the preserved Ko-fi card as a normal detail-information card.
