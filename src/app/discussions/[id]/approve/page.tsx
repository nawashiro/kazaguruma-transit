...
import { ModeratorCheck, PermissionError } from '@/components/discussion/PermissionGuards'
...
export default function PostApprovalPage() {
  ...
  const loadData = useCallback(async () => {
    ...
    setIsLoading(true)
    try {
      const [discussionEvents, postsEvents, approvalsEvents] = await Promise.all([
  ...
  if (!discussion) {
    return (
      <div className="container mx-auto px-4 py-8">
        <div className="text-center">
          <h1 className="text-2xl font-bold mb-4">ディスカッションが見つかりません</h1>
                    ...
                    <div className="card-body">
                      <div className="flex justify-between items-start mb-4">
                        <div className="flex items-center gap-2">
                          <span className="badge badge-warning badge-sm">未承認</span>
                          {post.busStopTag && (
                      ...
                      <div className="flex justify-between items-center">
                        <span className="text-xs font-mono text-gray-500">
                          投稿者: {post.authorPubkey.slice(0, 8)}...{post.authorPubkey.slice(-8)}
                        </span>
                  ...
                  return (
                    <div key={post.id} className="card bg-base-100 shadow-sm border border-gray-200 dark:border-gray-700">
                      <div className="card-body">
                        <div className="flex justify-between items-start mb-4">
                          <div className="flex items-center gap-2">
                            <span className="badge badge-success badge-sm">承認済み</span>
                            {post.busStopTag && (
                              <span className="badge badge-outline badge-sm">{post.busStopTag}</span>
                            )}
                          </div>
                        ...
                        <div className="flex justify-between items-center">
                          <div className="text-xs text-gray-500">
                            <div>投稿者: {post.authorPubkey.slice(0, 8)}...{post.authorPubkey.slice(-8)}</div>
                            <div>承認者: {post.approvedBy?.length || 0}人</div>
                            {post.approvedAt && (
                              <div>承認日時: {formatRelativeTime(post.approvedAt)}</div>
                            )}
                          </div>
