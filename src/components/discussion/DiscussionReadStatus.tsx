...
export function DiscussionReadStatus({ isLoading, completionReason, hasData, onReload, approvalState }: DiscussionReadStatusProps) {
  ...
  return <div role="status" aria-live="polite" className="alert alert-warning my-4"><span>{message}</span>{onReload && <button type="button" aria-label="再読み込み" className="btn text-base btn-outline min-h-[44px] rounded-full dark:rounded-sm" onClick={onReload}><span className="ruby-text">再読み込み</span></button>}</div>;
}
