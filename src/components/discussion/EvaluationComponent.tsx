...
export function EvaluationComponent({ 
  ...
  const availablePosts = filterUnevaluatedPosts(posts.filter(p => p.approved), userEvaluations)
  const displayPosts = isRandomOrder ? shuffleArray(availablePosts) : availablePosts
    ...
    <div className="space-y-4">
      ...
      <div className="bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-lg p-6">
        {currentPost.busStopTag && (
          <div className="mb-3">
            <span className="badge badge-primary badge-sm">{currentPost.busStopTag}</span>
          </div>
        )}
        ...
        <div className="flex gap-4 justify-center">
          <button
            ...
            className={`btn btn-success btn-lg flex-1 max-w-xs ${
          ...
          <button
            onClick={() => handleEvaluate(currentPost.id, '-')}
            disabled={evaluatingPost !== null}
            className={`btn btn-error btn-lg flex-1 max-w-xs ${
              evaluatingPost === currentPost.id ? 'loading' : ''
            }`}
          >
            {evaluatingPost === currentPost.id ? '' : (
      ...
      {remainingCount > 1 && (
        <div className="progress progress-primary w-full">
          <div 
            ...
            style={{ width: `${((currentIndex + 1) / limitedPosts.length) * 100}%` }}
          />
        </div>
      )}
    </div>
