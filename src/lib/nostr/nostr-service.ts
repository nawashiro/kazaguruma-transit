...
export class NostrService {
  ...
  async publishSignedEvent(signedEvent: Event): Promise<boolean> {
    try {
      ...
      console.error('Failed to publish signed event:', error)
      return false
    }
  }
  ...
  createDeleteEvent(targetEventId: string): Omit<Event, 'id' | 'sig' | 'pubkey'> {
    return {
      kind: 5,
      created_at: Math.floor(Date.now() / 1000),
...
export const createNostrService = (config: NostrServiceConfig): NostrService => {
